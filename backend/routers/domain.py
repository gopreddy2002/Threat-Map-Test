...
async def analyze_domain(payload: ScanCreate, db: Session = Depends(get_db)):
    try:
        ...
        try:
            ...
            urlscan_task = asyncio.wait_for(urlscan_service.search_indicator(domain, "domain"), timeout=6.0)
            otx_task = asyncio.wait_for(alienvault_service.get_indicator_report(domain, "domain"), timeout=6.0)

            # Local OSINT tasks
            dns_task = asyncio.wait_for(osint_service.get_dns_records(domain), timeout=4.0)
            whois_task = asyncio.wait_for(osint_service.get_whois_data(domain), timeout=6.0)
            ssl_task = asyncio.wait_for(osint_service.get_ssl_metadata(domain), timeout=4.0)
            
            # Advanced Analytical Tasks
            whoisjson_task = asyncio.wait_for(whoisjson_service.get_domain_data(domain), timeout=2.0)
            domainscan_task = asyncio.wait_for(domainscan_service.get_scan_data(domain), timeout=2.0)
